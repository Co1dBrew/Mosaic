...
| **TD-9** iOS 上没有 zh-Hans 句向量模型 | **实测可用性矩阵**（iPhone Air · iOS 26 模拟器，`NLEmbeddingAvailabilityTests`）：`zh-Hans ❌` · `zh-Hant ❌` · `ja ❌` · **`en ✅ 512 维`**。同样的代码在 macOS 上拿得到 zh-Hans（640 维），Golden Set 实测就跑在它之上。**真机尚未验证 —— Week 6 真机复测时确认。** 在此之前 iOS 上只有 keyword 路可用，Developer Mode 会显示 `failed(本机没有可用的本地句向量模型)`。**不拿英文模型顶替**：换语言就是换向量空间，中文笔记配英文模型排出来的顺序没有意义 |
