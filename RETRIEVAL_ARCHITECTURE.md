...
# §28 收口期的两个产品缺陷（2026-09-01）

> §27 那四件事是**真机**发现的。这两件不需要真机 ——
> 它们是用户把 App 当 App 用了一遍之后提出来的。
>
> 这本身是结论：**自动化测试覆盖的是「功能做没做对」，
> 覆盖不了「用起来是不是这么回事」。** 当时 12 条 UI 流程全绿，
> 只是没有人做过「点了新建又什么都不写」和
> 「在设置里输入完然后想点下面那个开关」这两个动作 ——
> 因为写测试的人是照着**功能清单**写的，不是照着**人**写的。

## 28.1 新建笔记会留下「未命名笔记」

点「新建」立刻 `insert` + `save` 一条 `Card`。用户什么都不写就返回，
笔记流里就留下一行「未命名笔记」——
**每一次误触悬浮按钮产生一条，而清理要用户自己去长按删除。**

### 判定放在内核，因为它只能有一处

`NoteDraftPolicy.hasMeaningfulContent` 是纯函数，不碰 SwiftData / SwiftUI。
放内核不只是为了可测，更是为了**唯一** —— 两处实现迟早会在
「只有一张图片的笔记算不算空」这种问题上分叉，而分叉的后果是删掉用户刚拍的照片。

有效内容 = 标题 ∪ 标签 ∪ 任意非空内容块（文字·图片·录音·文档·链接）∪ OCR/转写文本。
块的判空复用 `NoteListPresentation.isEmptyNote` —— 「这个块算不算空」
在列表副标题、摘要触发、草稿判定三处必须是同一个答案。
空白字符（`" "` `"\n"` `"\t"` `"\u{00A0}"`）trim 后判空。

### 为什么是「先建后删」而不是「延迟落盘」

延迟落盘（草稿态编辑器，有内容才 insert）是更干净的架构。它要动的是
`@Bindable var card: Card`、`AppRoute` 持有的对象、以及每一个内容块的写入路径 ——
**在收口阶段做这件事，风险远大于收益。**

「建了再删」产品语义等价、改动面小得多，**前提是清理必须完整**。
所以它走的是删除笔记那条**既有**路径（媒体文件 → Card → `noteWasDeleted` 清 derived），
不是新写一遍 —— 重写一遍就等于再制造一次 §27 里刚修过的 orphan derived data。

### 三个必需的限定条件

1. **`isNewDraft`。** 用户打开老笔记清空再退出，**不能**替他删。
   「清空」和「删除」是两个意图，而删除入口本来就在 `⋯` 菜单里一步可达。
2. **`onDisappear` ≠ 「这一屏走了」。** 在 `NavigationStack` 里往上推一屏
   （笔记页 →「设置」）同样触发它。摘要生成走这条路只是白花一次钱，
   **删除走这条路是丢数据**，所以显式挡住。
3. **启动清扫**（`EmptyDraftSweep`）。App 在空笔记里被划掉时 `onDisappear` 跑不到。
   清扫用同一个内核判据，**不看时间窗口** —— 「有时候清有时候不清」
   比不清更糟，因为用户无法预测。

## 28.2 设置页输入完成后键盘不收起

11 个输入控件（Base URL ×2 / 模型名 ×2 / API Key / Embedding 模型·维度·URL /
STT 模型·URL·Key），输入完成后键盘全都留在屏幕上。
表单下面还有 Toggle 和按钮，于是用户要么盲点被挡住的区域，要么退出这一屏。

### 不用 `UIApplication.endEditing`

那是一句到处能调也到处被调的全局命令：它**绕过 SwiftUI 的焦点系统**，
于是 `@FocusState` 与真实焦点不一致，而不一致的那一刻没有任何报错。
更实际的问题是它**不可组合** —— 每个 View 复制一遍，下一个新增的输入框一定会漏。

改用 `@FocusState` 的枚举绑定：焦点是**一个值**，收起键盘就是把它置 `nil`。
枚举而不是 Bool —— 两个输入框之间切换时 Bool 会先经过一次「无焦点」，键盘会闪。

### 三条退出路径对应三种不同的用户意图

| 路径 | 实现 | 覆盖 | 用户在想什么 |
|---|---|---|---|
| 键盘「完成」 | `ToolbarItemGroup(placement: .keyboard)` | **全部** | 我填好了 |
| 软键盘 Return | `.submitLabel(.done)` + `.onSubmit` | 单行文本 / URL / 密码 | 这一项填好了 |
| 滑动列表 | `.scrollDismissesKeyboard(.interactively)` | 全部 | 我想看下面 |

第一条是**不可省的**：`Embedding 维度` 用 `numberPad`，
那个键盘上**根本没有 Return 键**。只做 `.onSubmit` 的话它永远关不掉。

### 键盘收起 ≠ 取消输入

这个修饰符不碰数据。设置项全部即时保存（`SettingsStore` 直写 `UserDefaults` /
Keychain），在收起键盘时顺手做一次「提交」或「回滚」都会改变既有语义。
真机用例因此同时断言**值还在**。

### 判据必须是系统键盘，不是状态变量

UI 用例断的是 `app.keyboards` 真的消失（`waitForNonExistence`，因为收起有动画）。
断 `@FocusState == nil` 等于**用 App 自己说的话去验证 App 自己的行为**，
那样即使键盘没关掉用例也会绿。
