...
端到端 hybrid（keyword + 本地嵌入 + 余弦 + RRF）—— **同一台设备跑了两次**：

| chunks | 第一次 P50 / P95 | 第二次 P50 / P95 |
|---|---:|---:|
| 1 000 | 8.20 / 9.62 ms | 16.62 / 18.59 ms |
| 5 000 | 24.96 / 31.17 ms | 32.24 / 41.59 ms |
| **20 000** | **92.53 / 119.30 ms** | **113.03 / 143.23 ms** |

**两次都在 250 ms 预算内**，所以结论（不上 ANN）成立。但必须记下来的是
**跑次间方差很大**：20k 的 P95 从 119 ms 到 143 ms（+20%），1k 甚至差了近一倍。

`test0_environment` 断言的是**开跑时**热状态为 `nominal`，而 `test4` 跑在整套用例
之后 —— 设备到那时已经热起来了。**所以真机延迟不能只跑一次就当定值**：
要么每档之间放凉得更久，要么报区间而不是点值。这一节现在报的是区间。

Mac release 同口径是 79.5 ms，真机约慢 1.5–1.8 倍。
