...
| Metric B-local 端到端 P50 / P95 ⚠️ | 8.20–16.62 / 9.62–18.59 | 24.96–32.24 / 31.17–41.59 | — | **92.53–113.03 / 119.30–143.23** |
| L2b 余弦检索 P50 / P95 | 0.64 / 0.70 | 1.64 / 1.93 | 2.85 / 3.16 | **5.45 / 5.86** |

本地 query 嵌入 P50 **6.21 ms** / P95 7.99 ms。20k 向量内存 **29.3 MB**。

⚠️ **Metric B-local 报的是两次跑批的区间，不是点值。** 同一台设备两次之间 20k 的 P95
从 119.30 到 143.23 ms（+20%），1k 差了近一倍 —— `test4` 跑在整套用例之后，
设备已经热起来。**真机延迟跑一次不能当定值。** 两次都在 250 ms 预算内，结论不变。
