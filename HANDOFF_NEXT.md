...
（CI `[0, 0]`），却把 P50 从 13.6 ms 推到 50.2 ms。
按现在的数据**该发的是 keyword**；语义路要成立得靠云端臂，而云端臂没跑。

检索质量的绝对水平也上了一个台阶：R@1 **0.175 → 0.478**（development）。
那不是调参调出来的，是修掉了两个词法路的缺陷（`GATE_POLICY.md` §6）。
...
| # | 事项 | 说明 |
|---|---|---|
| 4 | **`lexical_trap` 只有 0.095** | 21 条里只对了 2 条。这一类是 hybrid 存在的理由，现在它几乎没有被解决。**云端臂跑起来之后先看这一类** |
| 5 | **backlog 里的 5 条** | 它们是两个缺陷的证人里**至今仍失败**的那几条，明确不放进回归集（回归集是棘轮不是愿望清单）：`HG041` `LT01` `CR01` `NQ04` `NQ11`，说明见 `GATE_POLICY.md` §7 |
| 6 | **`ambiguous` 需要产品决定** | 「押金多久退」有两个都对的答案。现在的口径要求两条都召回（Recall 的严格定义）。产品上要不要一次返回两条、要不要提示「有两处相关」，这是个设计问题不是检索问题 |
| 7 | **8 个簇仍是 3–5 篇** | 那些簇上的 R@5 没有区分度。要么扩到 6 篇，要么在报告里不用它们支持 R@5 的结论 |
| 8 | **切分参数还能更松** | development 上 `r=0.25 f=1` 到 R@1 0.336、`r=0.15 f=2 +latin` 到 0.398，代价是负例克制率降到 80% / 70%。**等 abstention 能上生产再回来取**（`GATE_POLICY.md` §6.3） |
